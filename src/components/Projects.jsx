        ...
        {filteredProjects.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 items-start">
            {filteredProjects.map((project) => (
              ...
              >
                <div>
                  {/* Revealed Top Media on Hover (Expands smoothly from 0 height with 0.5s hold delay) */}
                  {project.media && (
                    <div className="max-h-0 opacity-0 group-hover:max-h-48 sm:group-hover:max-h-52 group-hover:opacity-100 transition-all duration-500 delay-500 group-hover:duration-300 group-hover:delay-0 ease-[cubic-bezier(0.25,1,0.5,1)] overflow-hidden relative bg-zinc-950">
                      <img
                        src={project.media}
                        alt={project.title}

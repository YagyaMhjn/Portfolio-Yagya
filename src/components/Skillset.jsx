      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3 sm:gap-4 mb-14">
          {filteredSkills.map((skill) => (
            <div
              key={skill.id}
              className="glass-card glass-panel-hover p-4 rounded-xl border border-white/[0.07] flex flex-col justify-between group"
            >
              <div className="flex items-start justify-between mb-2">
                <span className="text-[10px] font-mono uppercase tracking-wider text-zinc-400">
                  {skill.category}
                </span>
                <span className="text-[10px] px-1.5 py-0.5 rounded bg-white/[0.05] text-zinc-300 font-mono border border-white/[0.05]">
                  {skill.level || 'Expert'}
                </span>
              </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="glass-card p-5 rounded-xl border border-white/[0.06] flex items-start gap-3">
            <div className="p-2 rounded-lg bg-zinc-900 border border-white/10 text-white mt-0.5">
              <Cpu size={16} />
            </div>
            <div>

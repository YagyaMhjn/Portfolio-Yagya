...
import { Briefcase, GraduationCap, Download, Calendar, ArrowUpRight } from 'lucide-react';

export const Journey = () => {
  const { data } = usePortfolio();
  const { timeline, profile } = data;
  const [filterType, setFilterType] = useState('all'); // 'all' | 'experience' | 'education'
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-12">
          ...
          <div className="mt-8 flex flex-wrap justify-center gap-3">
            <a
              href="#contact"
              className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-zinc-900 hover:bg-zinc-800 text-zinc-200 border border-white/10 text-xs font-medium transition-all"
            >
          ...
          {filteredTimeline.map((item, idx) => {
            ...
            return (
              <div key={item.id || idx} className="relative pl-6 sm:pl-8 group">
                ...
                <div className="absolute -left-[9px] top-1.5 w-4 h-4 rounded-full bg-[#0a0a0c] border-2 border-zinc-600 group-hover:border-white transition-colors flex items-center justify-center">
                ...
                {/* Card */}
                <div className="glass-card glass-panel-hover p-6 rounded-2xl border border-white/[0.08]">
                  <div className="flex flex-wrap items-center justify-between gap-2 mb-2">
                    <div className="flex items-center gap-2">
                      <span className="p-1.5 rounded-lg bg-zinc-900 border border-white/[0.08] text-zinc-300">
